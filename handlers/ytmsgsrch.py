...
async def ytsearch(_, message: Message):
    try:
        if len(message.command) < 2:
            await message.reply_text("`/ytsearch` ʙɪʀ ᴀʀɢᴜᴍᴀɴᴀ ɪʜᴛɪʏᴀᴄɪ ᴠᴀʀ!")
            return
        query = message.text.split(None, 1)[1]
        m = await message.reply_text("**ᴀɴᴀʜᴛᴀʀ ᴋᴇʟɪᴍᴇɴɪᴢɪ ᴀʀᴀᴍᴀ** 😒")
        results = YoutubeSearch(query, max_results=4).to_dict()
        thumb = THUMB_URL
        i = 0
        text = ""
        while i < 4:
            text += f"📄**ʙᴀꜱʟɪᴋ:** `{results[i]['title']}`\n"
            text += f"  ↳**ꜱᴜʀᴇ:** `{results[i]['duration']}`\n"
            text += f"  ↳**ɢᴏʀᴜɴᴛᴜʟᴇᴍᴇ:** `{results[i]['views']}`\n"
            text += f"  ↳**ᴋᴀɴᴀʟ:** `{results[i]['channel']}`\n"
            text += f"  ↳**ᴜʀʟ:** https://youtube.com{results[i]['url_suffix']}\n\n"
            i += 1
        await m.delete()
